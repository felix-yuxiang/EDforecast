...
#         X = df.drop(columns=['Date','Number_Visits', 'holiday_name', 'normal day',"Age at time of death_Age at time of death, 0 to 44 years_x","Age at time of death_Age at time of death, 45 to 64 years_x","Age at time of death_Age at time of death, 65 to 84 years_x","Age at time of death_Age at time of death, 85 years and over_x"
# ,"Age at time of death_Age at time of death, 0 to 44 years_y","Age at time of death_Age at time of death, 45 to 64 years_y","Age at time of death_Age at time of death, 65 to 84 years_y","Age at time of death_Age at time of death, 85 years and over_y"])
       ...
       'TOTAL_RAIN', 'TOTAL_PRECIPITATION', 'HEATING_DEGREE_DAYS', 'COOLING_DEGREE_DAYS'])
        ...
        #### normalize the data
        X_transformed = MinMaxScaler(feature_range=(-1,1)).fit_transform(X_transformed)
...
def plot_train_test_lr(lr, train_loss, test_loss):
    plt.plot(lr, train_loss, label='train_loss')
    plt.plot(lr, test_loss, label='test_loss')
    plt.title('L1 Loss vs. Learning Rate')
    plt.legend()
    plt.savefig(f'results/train_test_loss_lr.png')
    plt.clf()
...
def main():
    ...
    # ### plot 
    # size = 10
    # plt.scatter(range(len(y_pred)), y_pred, label='Predicted values (y_pred)', alpha=0.5, s=size, color='blue')
    # plt.scatter(range(len(y_gt)), y_gt, label='Actual values (y_train)', alpha=0.5, s=size, color='orange')

    # plt.title('Neural Network: Plot of y_train vs. y_pred')
    # plt.xlabel('Shuffled Index')
    # plt.ylabel('Values')
    # plt.legend()
    # plt.tight_layout()
    # # Set y-axis range
    ...
    # plt.savefig('./plots/nn_result_train.png')
    # plt.clf()
    ...
    # test_loss, y_pred, y_gt  = test(net, test_dl, criterion)
    # print_log('Test L2 Loss: {:.6f}'.format(test_loss), log_path)

    # L1_loss, _,_ = test(net, test_dl, nn.L1Loss())
    # print_log('Test L1 Loss: {:.6f}'.format(L1_loss), log_path)
    
    lr = [1e-4, 5e-4, 1e-3, 5e-3, 1e-2]
    train_loss = [718.210449,459.747620,477.327240,462.494812 ,467.861938]
    test_loss = [718.533203,267.976135, 539.554199, 486.848572, 424.468109]
    plot_train_test_lr(lr, train_loss, test_loss)
    # plot(y_pred, y_gt, f'test_dl_{province}')

    # plt.scatter(range(len(y_pred)), y_pred, label='Predicted values (y_pred)', alpha=0.5, s=size, color='blue')
    # plt.scatter(range(len(y_gt)), y_gt, label='Actual values (y_test)', alpha=0.5, s=size, color='orange')

    # plt.title('Neural Network: Plot of y_test vs. y_pred')
    ...
    # plt.savefig('./plots/nn_result_test.png')
    # plt.clf()
