...
X = df.drop(columns=['Date','Number_Visits', 'holiday_name', 'normal day', 'is_weekend'])
...
rft = TransformedTargetRegressor(regressor = Lasso(),transformer = StandardScaler())
...
plt.title('Lasso: Plot of y_test vs. y_pred')
plt.xlabel('Index')
plt.ylabel('Values')
plt.legend()
